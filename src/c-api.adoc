...
Toolchains provide macros to detect which RISC-V intrinsics are supported, independent of whether the corresponding ISA extensions are currently enabled via command-line options (e.g., `-march`). These macros allow developers to detect toolchain capabilities at compile-time and write portable code that can use intrinsics when available.

*Note:* Intrinsic support is independent from assembler support. A toolchain may support intrinsics for an extension even if that extension is not enabled in the current compilation unit.
...
----
  ...
  // Toolchain supports Zbb scalar intrinsics like __builtin_riscv_orc_b_*
