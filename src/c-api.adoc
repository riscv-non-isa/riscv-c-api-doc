...
*Note:* Intrinsic support is independent from assembler support. A toolchain may support intrinsics for an extension even if that extension is not enabled in the current compilation unit. This is only available if intrinsics are ratified, in other word, experimental intrinsics do not have macro defined.
