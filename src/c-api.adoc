...
|`+__riscv_misaligned_avoid+` |1  | Misaligned accesses are not supported and could trap. (see <<__riscv_misaligned_fast_slow_avoid, `+__riscv_misaligned_{fast,slow,avoid}+`>>)
|===
...
=== +__riscv_misaligned_{fast,slow,avoid}+  todo
