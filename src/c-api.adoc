...
=== Intrinsic Support Detection Macros

Toolchains provide macros to detect which RISC-V intrinsics are supported, independent of whether the corresponding ISA extensions are currently enabled via command-line options (e.g., `-march`). These macros allow developers to detect toolchain capabilities at compile-time and write portable code that can use intrinsics when available. Macros are defined in their corresponding header files, e.g. __riscv_intrinsic_zbb, __riscv_intrinsic_zbc, etc. are defined in riscv_bitmanip.h, it can be used with __has_include macro to guard intrinsic usage.

*Note:* Intrinsic support is independent from assembler support. A toolchain may support intrinsics for an extension even if that extension is not enabled in the current compilation unit. This is only available if intrinsics are ratified, in other words, experimental intrinsics do not have macro defined.

==== Detection Macro Naming

Intrinsic support detection macros follow the naming pattern `+__riscv_intrinsic_<extension>+`, where `<extension>` is the extension name in lowercase. This applies to both scalar and vector intrinsics.

These macros are defined with the value `1` when the toolchain has intrinsic support for the corresponding extension.

Examples:

.Scalar Intrinsic Detection Macros
- `+__riscv_intrinsic_zbb+` - Zbb (Basic bit-manipulation) scalar intrinsics
- `+__riscv_intrinsic_zbc+` - Zbc (Carry-less multiplication) scalar intrinsics
- `+__riscv_intrinsic_zbkb+` - Zbkb (Bit-manipulation for cryptography) scalar intrinsics

.Vector Intrinsic Detection Macros
- `+__riscv_intrinsic_v+` - Base vector (V extension) intrinsics
- `+__riscv_intrinsic_zve32x+` - Zve32x vector intrinsics
- `+__riscv_intrinsic_zve32f+` - Zve32f vector intrinsics
- `+__riscv_intrinsic_zvbb+` - Zvbb (Vector bit-manipulation) intrinsics
- `+__riscv_intrinsic_zvbc+` - Zvbc (Vector carry-less multiplication) intrinsics

.Vendor Extension Detection Macros
- `+__riscv_intrinsic_xtheadbb+` - XTheadBb scalar intrinsics
- `+__riscv_intrinsic_xsfvcp+` - SiFive VCIX vector intrinsics
- `+__riscv_intrinsic_xandesvdot+` - Andes vector dot product intrinsics
...
When all sub-extensions of a composite extension have intrinsic support, a macro for the composite extension is also defined. These are defined conditionally using preprocessor directives.

Examples of scalar and vector composite extensions:

- `+__riscv_intrinsic_zkn+` - Defined when all of the following are supported: `zbkb`, `zbkc`, `zbkx`, `zkne`, `zknd`, `zknh`
- `+__riscv_intrinsic_zks+` - Defined when all of the following are supported: `zbkb`, `zbkc`, `zbkx`, `zksed`, `zksh`
...
|===
|*Type*            |*Description*
|`+int_fastN_t+`   |Fastest signed integer types with widths of at least `+N+` bits.
|`+int_leastN_t+`  |Smallest signed integer types with widths of at least `+N+` bits.
|`+uint_fastN_t+`  |Fastest unsigned integer types with widths of at least `+N+` bits.
...
|`+uint32_t __riscv_clmulh_32(uint32_t rs1, uint32_t rs2);+`               |`clmulh`           |Zbc, Zbkc (RV32)  |No emulation for RV64
|`+uint64_t __riscv_clmulh_64(uint64_t rs1, uint64_t rs2);+`               |`clmulh`           |Zbc, Zbkc (RV64)  |
|`+uint32_t __riscv_clmulr_32(uint32_t rs1, uint32_t rs2);+`               |`clmulr`           |Zbc (RV32)        |No emulation for RV64
...
=== V and Zv* Extension Intrinsics

Intrinsics for RISC-V Vectors (RVV) are documented in https://github.com/riscv-non-isa/riscv-rvv-intrinsic-doc[RISC-V Vector Intrinsics].
